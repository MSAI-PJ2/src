...
영역으로 바로 이동한다. "무엇을 답할지"는 counsel/policy.py, "어떻게 흘러가는지"는
counsel/flow.py 로 역할이 갈린다.
...
시스템 프롬프트 AI 에게 답변 전에 주는 지시문 (말투·역할·금지사항) — app/counsel/policy.py 구획 2
...
3. app/counsel/flow.py [구획 5]  상담 한 턴의 핵심 6단계
4. app/counsel/policy.py        정책·프롬프트·위기 (사람 편집 영역 전부)
```

## 사람이 편집하는 지점 — counsel/policy.py 한 파일

```text
라벨별 응답 정책       counsel/policy.py [구획 1]   POLICIES 테이블 (+ 저확신 강등 노브)
답변 스타일/프롬프트   counsel/policy.py [구획 2]   PERSONA·STYLE_RULES·LABEL_GUIDANCE
위기 메시지/핫라인     counsel/policy.py [구획 3]   + 위치 기반 DB 조회 작업 가이드
