...
`-- app/
    ...
    |   `-- flow.py        [기계장치]  구획: ①SSE 이벤트 ②요청 정리 ③RAG 선별 ④진입 스트림 ⑤핵심 흐름
...
위기 메시지/핫라인     respond/policy.py [구획 3]   지역 연락처 DB 는 .env HOTLINE_CONTAINER 로 켜기
로그인(Entra) 켜기     .env (AUTH_MODE=entra + ENTRA_CLIENT_ID + ENTRA_TENANT_ID/ISSUER)
튜닝 노브             .env (RAG_TOP_N · POLICY_MIN_CONFIDENCE — settings.py 참고)
```
...
- 위기 지역 연락처 DB 조회 구현 완료(잠금 상태) — `HOTLINE_CONTAINER` 만 채우면 켜짐.
  조회 실패/타임아웃이어도 위기 응답은 전국 공통 창구로 반드시 나간다 (respond/policy.py 구획 3)
