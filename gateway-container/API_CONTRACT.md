...
- **중립 초기값**: 새 세션은 생성 시 중립 기본 이름 `"새 대화"` 를 받는다(내용 기반 아님 —
  상담 도메인 민감정보가 라벨로 노출되지 않게). 사용자가 이름을 붙이기 전까지 이 값이 뜬다.
- **폴백 순서**: `name`(신규 세션은 항상 있음) → `preview`(첫 발화 40자, 옛 세션용) → 날짜.
  어느 경우도 프론트는 **원본 전체 ID(코드)를 노출하지 않는다**(구분은 뒤 4~6자만 붙임).
- **설정/변경**: `PATCH /v1/sessions/{session_id}` 바디 `{"name": "..."}`. 서버가 한 줄로
  정규화(줄바꿈·연속 공백 축약)하고 60자로 자른다. name 을 빈 문자열/공백/`null` 로 보내면
  **이름을 지우는 게 아니라 중립 기본값(`"새 대화"`)으로 되돌린다** — 세션은 항상 이름을 가진다.
  응답은 갱신된 세션 스냅샷(`name` 포함).
- **소유권**: 로그인 사용자(user_id) 소유 세션만 이름 변경 가능. 남의 세션·없는 세션은 404.
- `GET /v1/sessions`(내 세션 목록)과 `GET /v1/sessions/{id}` 응답에 `name` 포함. 이 기능 도입
  이전에 만들어진 옛 세션만 `name` 이 없어(`null`) preview/날짜 폴백으로 처리된다(마이그레이션 불필요).
...
kakao           카카오톡 캡쳐 — 화자 분리. '나'(내담자)가 진단 대상, 상대방은 분류 맥락
```

**분류 맥락(옵션2, 2026-07-06):** kakao 는 '나' 발화를 진단하되, **상대방 발화를 맥락으로
함께 분류기에 넣는다**(화자 라벨 포맷 `상대방: …\n나: …`, 뒤에서부터 `CLASSIFY_CONTEXT_MAX_CHARS`
예산). 이유: 모호한 '나' 발화가 직전 상대방 발화 맥락으로 정확히 분류된다(실측: "거봐 역시
나랑 얘기하기 싫은거잖아" → 나만=불충분, 맥락=성급한 판단 0.99). **저장·표시되는 사용자 턴
텍스트는 여전히 '나' 발화만**이며, 맥락 사용은 내부 처리다(과정 블랙박스). 맥락으로 분류한
턴은 `analysis.merge_trigger="ocr_conversation"`·`context_merged=true` 로 표시된다. 상대방
발화가 없으면 '나' 발화만으로 분류(폴백). 상대방 발화에 라벨을 붙이지는 않는다(컨텍스트 오염 방지).
