...
async def emit_tts_event(session_id: str, text: str, options: dict | None):
    """Build a TTS SSE event.

    Returns Azure Speech audio as base64 for the MVP. A later Blob-backed
    implementation can replace audio_base64 with audio_url/blob_ref without
    ...
    try:
        result = await _speech.synthesize_tts(text, options)
        event.update(result)
...
async def respond_stream(
    ...
    if not safety["safe"]:
        ...
        if tts and tts.get("enabled"):
            yield sse(await emit_tts_event(session_id, payload.get("message", ""), tts))
        yield sse({"type": "done", "session_id": session_id})
        return
    ...
    if tts and tts.get("enabled"):
        yield sse(await emit_tts_event(session_id, assistant_text, tts))
